...
function actionFormatter(value, row, index) {
    ...
    var result = "";
    //result += "<button class='btn btn-outline-success btn-sm ch' type='button' onclick=\"AAById('" + id + "','" + row + "')\">均摊</button>";
    result += "<button class='btn btn-outline-danger btn-sm ch' type='button' onclick=\"DeleteByIds('" + id + "','" + row + "')\">删除</button>";
    //result += "<button class='btn btn-outline-primary btn-sm ch mx-2' type='button' onclick=\"EditViewById('" + row + "','" + id + "')\">创建团队</button>";
...
function AAById(row, index) {
    ...
    $("#name").val(data_json[index].name);
    $("#name").removeAttr("disabled", "true");  // 删除禁止修改属性
    $("#leader").val(data_json[index].leader);
    $("#leader").removeAttr("readonly");
    $("#tfee").val(data_json[index].tfee);
    $("#tfee").removeAttr("readonly");
    $("#fee").val(data_json[index].fee);
    $("#fee").removeAttr("readonly");
    $("#people").val(data_json[index].people);
...
function DeleteByIds(index, row) {
    ...
    axios.delete("/joiners/" + data_json[index].id.toString()).then((res) => {
