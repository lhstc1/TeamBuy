...
function actionFormatter(value, row, index) {
    ...
    var result = "";
    //result += "<button class='btn btn-outline-success btn-sm ch' type='button' onclick=\"ChooseById('" + id + "','" + row + "')\">选择活动</button>";
    result += "<button class='btn btn-outline-primary btn-sm ch mx-2' type='button' onclick=\"EditViewById('" + row + "','" + id + "')\">修改</button>";
...
function EditViewById(row, index) {
    ...
    $("#update_span").val(data_json[index].span);
    $("#update_span").removeAttr("readonly");
    $("#update_fee").val(data_json[index].fee);
    $("#update_fee").removeAttr("readonly");
    $("#update_introduction").val(data_json[index].introduction);
    $("#update_introduction").removeAttr("readonly");
...
function DeleteByIds(index, row) {
    ...
    axios.delete("/activities/" + data_json[index].id.toString()).then((res) => {
